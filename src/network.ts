// network.ts — Network configuration, state management, and seed helpers.
...
export function getNetworkConfig(network?: NetworkId | null): NetworkConfig {
  const target = (network && isNetworkId(network)) ? network : 'undeployed';
  return NETWORK_CONFIGS[target];
}
...
export function saveState(state: NetworkState, opts: FsOptions = {}): void {
  const p = statePath(opts);
  // Atomic write: tmp file then rename (POSIX safe)
  const tmp = `${p}.tmp-${process.pid}-${Date.now()}`;
  fs.writeFileSync(tmp, `${JSON.stringify(state, null, 2)}\n`);
  fs.renameSync(tmp, p);
}

export function parseNetworkFlag(argv: string[] = process.argv): NetworkId | null {
...
// CLI entry point — only activates when run directly via tsx, not when imported.
